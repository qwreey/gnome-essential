...
const { getShadowSize, PointerMovePreventer, FakePointer, resizingOps, Maid, PointerUtil, getOffset, applyOffset, WindowMover, getResizeAnimationSize } = Me.imports.libs.utility
...
var Wireframe = class Wireframe {
	...
	grapBegin(_d ,window, op) {
		...
		this.#fakePointer.setPosition(cursorX,cursorY)
		this.#pointerMovePreventer.lockMoveAt(cursorX,cursorY)
		...
		global.window_group.insert_child_above(this.#draggedWindowClone,windowActor)
		windowActor.hide()
		...
		this.#draggedWindowClone.add_effect_with_name("blur",this.#blurEffect)
	...
	grapEnd(_d ,window, op) {
		...
		this.#draggedWindowActor.show()
		this.#draggedWindowClone.hide()
	...
	updateDraggingWidget(x,y,width,height) {
		...
		const { cloneGoalScaleX, cloneGoalScaleY, cloneGoalX, cloneGoalY } = getResizeAnimationSize(this.#draggedPos,x,y,width,height)
		// if (this.#blurEffect) this.#blurEffect.sigma = Math.min(Math.abs(width * height - this.#draggedPos.frameWidth * this.#draggedPos.frameHeight) / 142**2,16)
		this.#draggedWindowClone.scaleX = cloneGoalScaleX
		this.#draggedWindowClone.scaleY = cloneGoalScaleY
		this.#draggedWindowClone.x = cloneGoalX
		this.#draggedWindowClone.y = cloneGoalY
	}
